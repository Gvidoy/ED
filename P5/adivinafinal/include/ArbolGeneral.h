  	...
  	*/
 	  iter_preorden& operator ++ () {
   		...
   		if ( it->izqda != 0 ) {
  			it = it->izqda;
  			level = level+1; 
			//cout << "ITER_PREORDEN :: HijoIzquierda de it >> " << it << " LEVEL = " << level << endl;
  		} 
  		else {
				if ( it->drcha != 0 ) {
	  				it = it->drcha;
					//cout << "ITER_PREORDEN :: HermaDerecha de it >> "<< it << " LEVEL = " << level << endl;
				}
				else {
	 				if ( it->padre != 0 ) {
   					while  ( it->padre != 0 && it->drcha == 0) {
  						it = it->padre;
    					level = level-1;
							//cout << "ITER_PREORDEN :: SIN HIJOS, Padre de it >> "<< it << " LEVEL = " << level << "\n it->drcha = " << it->drcha <<  endl;
   					}
   					if ( it->drcha != 0 )
   						it = it->drcha;
   					else {
   					...
   					it = 0;
   					level = -1;
						//cout << "ITER_PREORDEN :: SOY LA RAIZ >> " << it << " LEVEL = " << level << endl;
	 				}
				}	 
  		}
  		return *this;
